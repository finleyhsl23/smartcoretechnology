// POST /api/sitelens/create-api-key { companyId, label }
// Creates a read-only SiteLens export API key. The raw key is returned once
// in the response and never stored — only its SHA-256 hash is persisted, so
// it can be looked up (but not recovered) by functions/api/sitelens/api-export.js.
...
export async function onRequestPost({ request, env }) {
  ...
  const allowed = await hasPermission(env, caller.token, companyId, 'sitelens.manage_settings');
  if (!allowed) return json({ error: 'You do not have permission to manage SiteLens settings.' }, 403);
  ...
  try {
    await sbPost(env, '/sitelens_api_keys', {
