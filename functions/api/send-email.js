 ...
 * POST /api/send-email
 *
 * Sends welcome + admin receipt emails after a confirmed payment.
...
.guide-box{font-size:14px;line-height:1.8;color:#8a8a9e}
.guide-box p{margin-bottom:14px}.guide-box p:last-child{margin-bottom:0}
.guide-box strong{color:#c0c0d4}
...
// ---------------------------------------------------------------------------
// Welcome email — sent to the customer
// ---------------------------------------------------------------------------
function welcomeHtml(o, modules, es) {
  const regular = modules.filter(m => m.slug !== 'smartcore-core');
  const date = new Date(o.created_at || Date.now()).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' });
  ...
  const modList = [
    ...
    ...regular.map(m => `<div class="mod-row"><div class="mod-left"><div class="mod-dot"></div><span class="mod-name">${esc(m.name)}</span></div><span class="mod-price">${fmt(m.price)}/mo</span></div>`),
  ].join('');
  ...
  return emailShell(
    ...
    <p class="intro">Your payment is confirmed and your SmartCore workspace is live. Below you'll find everything you need to get up and running today.</p>
    ...
    <div class="section">
      ...
      <div class="kv"><span class="kv-k">Order date</span><span class="kv-v">${date}</span></div>
    </div>
    ...
    <div class="section">
      ...
      ${(o.annual_discount_amount > 0) ? `<div class="disc-row"><span>Annual billing discount (8%)</span><span class="disc-val">−${fmt(o.annual_discount_amount)}</span></div>` : ''}
      <div class="total-row"><span>Total</span><span>${fmt(o.total)}/mo</span></div>
    </div>

    <div class="section">
      <div class="section-label">Access Your Workspace</div>
      <p style="font-size:14px;color:#8a8a9e;margin-bottom:16px">Your account is ready. Click below to log in and start using SmartCore immediately.</p>
    ...
    <div class="section">
      ...
      <div class="guide-step">
        ...
        <div class="guide-text"><strong>Sign in to your workspace</strong>Use the button above to open SmartCore. Sign in with the email address registered on this order. If this is your first time, you may be prompted to set a password.</div>
      </div>
      <div class="guide-step">
        <div class="guide-num">2</div>
        <div class="guide-text"><strong>Explore your dashboard</strong>Your home screen gives you a live overview of your business. All activated modules appear in the left-hand navigation — everything is pre-configured and ready to use from day one.</div>
      </div>
      <div class="guide-step">
        <div class="guide-num">3</div>
        <div class="guide-text"><strong>Invite your team</strong>Go to <strong>Settings → Team Members</strong> to add colleagues. Assign roles, set permissions, and manage access — your team can be onboarded in minutes.</div>
      </div>
      <div class="guide-step">
        <div class="guide-num">4</div>
        <div class="guide-text"><strong>Expand anytime</strong>Need more? Browse the full SmartCore module catalogue at <a href="${SITE}/modules" style="color:#5b8fff">${SITE.replace('https://', '')}/modules</a> and add new modules to your subscription whenever you're ready.</div>
      </div>
    </div>
    ...
    <p class="small">Questions or need a hand getting set up? Our team is here for you — email us at <a href="mailto:${supportEmail}">${supportEmail}</a> and we'll get back to you promptly.</p>
...
// ---------------------------------------------------------------------------
// Receipt email — sent to SmartCore admin
// ---------------------------------------------------------------------------
function receiptHtml(o, modules) {
  const regular = modules.filter(m => m.slug !== 'smartcore-core');

  const modList = [
    `<div class="mod-row"><div class="mod-left"><div class="mod-dot green"></div><span class="mod-name">SmartCore Core</span></div><span class="mod-price free">Included free</span></div>`,
    ...regular.map(m => `<div class="mod-row"><div class="mod-left"><div class="mod-dot"></div><span class="mod-name">${esc(m.name)}<a href="${SITE}/shop/" class="mod-link">View in Shop →</a></span></div><span class="mod-price">${fmt(m.price)}/mo</span></div>`),
  ].join('');

  return emailShell(
    `New order from ${o.company_name} — ${o.order_reference}`,
    `<span class="tag tag-blue">💳 New Order</span>
    ...
    <div class="section">
      ...
      <div class="kv"><span class="kv-k">Billing type</span><span class="kv-v">${o.billing_type === 'yearly' ? 'Annual' : 'Monthly'}</span></div>
    </div>

    <div class="section">
      <div class="section-label">Modules (${regular.length + 1} total)</div>
      ${modList}
      ${(o.discount_amount > 0) ? `<div class="disc-row"><span>Package discount</span><span class="disc-val">−${fmt(o.discount_amount)}</span></div>` : ''}
      ${(o.annual_discount_amount > 0) ? `<div class="disc-row"><span>Annual billing discount (8%)</span><span class="disc-val">−${fmt(o.annual_discount_amount)}</span></div>` : ''}
      <div class="total-row"><span>Total</span><span>${fmt(o.total)}/mo</span></div>
