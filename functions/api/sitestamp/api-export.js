// GET /api/sitestamp/api-export
// Header: X-API-Key: sl_live_...
// Read-only export for external tools: active projects and recent media
// (with short-lived signed URLs) for the key's company. This is SiteStamp's
...
export async function onRequestGet({ request, env }) {
  ...
  let keyRows;
  try {
    keyRows = await sbGet(env, `/sitestamp_api_keys?key_hash=eq.${keyHash}&select=*&limit=1`);
  } catch {
  ...
  await sbPatch(env, `/sitestamp_api_keys?id=eq.${keyRow.id}`, { last_used_at: new Date().toISOString() }).catch(() => {});

  const companyId = keyRow.company_id;
  const [projectRows, mediaRows] = await Promise.all([
    sbGet(env, `/sitestamp_projects?company_id=eq.${companyId}&status=eq.active&select=id,name,client_name,status,city,postcode,created_at&order=created_at.desc`),
    sbGet(env, `/sitestamp_media?company_id=eq.${companyId}&select=id,project_id,media_type,caption,taken_at,latitude,longitude,storage_path&order=taken_at.desc&limit=100`),
  ]);
...
async function createSignedUrl(env, path) {
  try {
    const res = await fetch(`${env.SUPABASE_URL}/storage/v1/object/sign/sitestamp-media/${path}`, {
      method: 'POST',
      headers: {
