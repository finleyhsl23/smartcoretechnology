...
export async function onRequest(context) {
  try {
    ...
    const { sub: portalUserId, tid: tenantId, cid: contactId } = claims;
    ...
    const [userRows] = await Promise.all([
      db(`crm_portal_users?id=eq.${portalUserId}&select=id,email,name,status,crm_contact_id,crm_contacts(first_name,last_name,job_title,crm_companies(name))&limit=1`),
    ]);
    const user = userRows?.[0];
    if (!user || user.status === "suspended") return json({ ok: false, error: "Access denied" }, 403);

    if (section === "messages") {
      if (!contactId) return json({ ok: true, messages: [] });
      const msgs = await db(`crm_messages?crm_contact_id=eq.${contactId}&tenant_id=eq.${tenantId}&order=created_at.asc`);
      // Mark customer messages as read (fire-and-forget)
      fetch(`${SUPABASE_URL}/rest/v1/crm_messages?crm_contact_id=eq.${contactId}&sender_type=eq.staff&read_at=is.null`, {
...
export async function onRequestPost(context) {
  try {
    ...
    const { sub: portalUserId, tid: tenantId, cid: contactId, name } = claims;
    if (!contactId) return json({ ok: false, error: "No linked contact" }, 400);
