export async function onRequestPost(context) {
  try {
    ...
    const encrypted_payload = await encryptPayload(context, payload);
    await serviceFetch(context, "/rest/v1/credentials_vault", { method: "POST", body: JSON.stringify({ name: payload.name, encrypted_payload, archived: false }) });
    return json({ ok: true });
