// POST /api/sitesnap/webhook-notify { companyId, eventType, payload }
...
export async function onRequestPost({ request, env }) {
  ...
  let hooks;
  try {
    hooks = await sbGet(env, `/sitesnap_webhooks?company_id=eq.${companyId}&is_active=eq.true&event_types=cs.{${eventType}}&select=*`);
  } catch {
  ...
  for (const hook of hooks) {
    ...
    try {
      ...
      const res = await fetch(hook.target_url, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-SiteSnap-Signature': signature, 'X-SiteSnap-Event': eventType },
        body: bodyText,
      });
    ...
    await sbPatch(env, `/sitesnap_webhooks?id=eq.${hook.id}`, { last_fired_at: new Date().toISOString(), last_status: status }).catch(() => {});
  }
