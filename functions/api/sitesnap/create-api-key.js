// POST /api/sitesnap/create-api-key { companyId, label }
// Creates a read-only SiteSnap export API key. The raw key is returned once
// in the response and never stored — only its SHA-256 hash is persisted, so
// it can be looked up (but not recovered) by functions/api/sitesnap/api-export.js.
...
export async function onRequestPost({ request, env }) {
  ...
  const allowed = await hasPermission(env, caller.token, companyId, 'sitesnap.manage_settings');
  if (!allowed) return json({ error: 'You do not have permission to manage SiteSnap settings.' }, 403);
  ...
  try {
    await sbPost(env, '/sitesnap_api_keys', {
