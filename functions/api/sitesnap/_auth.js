// Shared auth/authorization helpers for SiteSnap Cloudflare Pages Functions.
...
// `permission` within `companyId` — forwards to sitesnap_has_permission as
...
export async function hasPermission(env, token, companyId, permission) {
  try {
    const res = await fetch(`${baseUrl(env)}/rest/v1/rpc/sitesnap_has_permission`, {
      method: 'POST',
