-- ============================================================================
-- SmartCore Convoy — grant access to SmartCore Technology LTD
...
-- (convoy_module_enabled / requireConvoyAccess). Neither table has a unique
-- constraint on (company_id, module_slug/module_key), so idempotency is
-- guarded explicitly rather than via ON CONFLICT — same pattern used for the
-- SiteStamp and Flexi internal-grant migrations.
-- ============================================================================

INSERT INTO public.smartcore_core_purchased_modules (company_id, module_slug, module_name, billing_type, status)
SELECT 'b9298a9b-a910-4ba9-9a57-72db43b3b3d7', 'convoy', 'Convoy', 'monthly', 'active'
WHERE NOT EXISTS (
  SELECT 1 FROM public.smartcore_core_purchased_modules
  WHERE company_id = 'b9298a9b-a910-4ba9-9a57-72db43b3b3d7' AND module_slug = 'convoy'
);

INSERT INTO public.company_modules (company_id, module_key, enabled)
SELECT 'b9298a9b-a910-4ba9-9a57-72db43b3b3d7', 'convoy', true
WHERE NOT EXISTS (
  SELECT 1 FROM public.company_modules
  WHERE company_id = 'b9298a9b-a910-4ba9-9a57-72db43b3b3d7' AND module_key = 'convoy'
);
