-- ============================================================================
-- SmartCore SiteSnap — Migration 1: Foundation
...
-- key: sitesnap. Tables are prefixed sitesnap_ in the public schema, mirroring
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.sitesnap_permission_grants (
...
CREATE INDEX IF NOT EXISTS sitesnap_permission_grants_employee_idx ON public.sitesnap_permission_grants(employee_id);
CREATE INDEX IF NOT EXISTS sitesnap_permission_grants_company_idx ON public.sitesnap_permission_grants(company_id);

ALTER TABLE public.sitesnap_permission_grants ENABLE ROW LEVEL SECURITY;

CREATE POLICY sitesnap_permission_grants_select_own_company ON public.sitesnap_permission_grants
  FOR SELECT USING (
    company_id IN (SELECT ce.company_id FROM public.core_employees ce WHERE ce.auth_user_id = auth.uid())
  );

CREATE POLICY sitesnap_permission_grants_write_admins ON public.sitesnap_permission_grants
  FOR ALL USING (
    company_id IN (
...
AS $$
  SELECT CASE p_role
    ...
    WHEN 'manager' THEN ARRAY[
      'sitesnap.view_projects','sitesnap.manage_projects','sitesnap.capture_media','sitesnap.delete_media',
      'sitesnap.manage_checklists','sitesnap.manage_tasks','sitesnap.export_reports'
    ]
...
CREATE OR REPLACE FUNCTION public.sitesnap_current_employee(p_company_id uuid)
...
CREATE OR REPLACE FUNCTION public.sitesnap_current_employee_id(p_company_id uuid)
RETURNS uuid
LANGUAGE sql
STABLE
SECURITY DEFINER
  ...
  SELECT EXISTS (
    ...
    WHERE ce.auth_user_id = auth.uid()
      ...
      AND (
        ...
        OR EXISTS (
          SELECT 1 FROM public.sitesnap_permission_grants g
...
CREATE OR REPLACE FUNCTION public.sitesnap_module_enabled(p_company_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT COALESCE(
    (SELECT cm.enabled FROM public.company_modules cm
     WHERE cm.company_id = p_company_id AND cm.module_key = 'sitesnap'
     LIMIT 1),
    false
  );
$$;
  ...
  SELECT CASE
    ...
    ELSE (
      SELECT array_agg(DISTINCT p) FROM (
        SELECT unnest(public.sitesnap_default_permissions(ce.role)) AS p
        UNION
        SELECT g.permission FROM public.sitesnap_permission_grants g
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.sitesnap_audit_logs (
  id               uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id       uuid NOT NULL REFERENCES public.smartcore_core_companies(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS sitesnap_audit_logs_company_idx ON public.sitesnap_audit_logs(company_id, created_at DESC);
CREATE INDEX IF NOT EXISTS sitesnap_audit_logs_entity_idx ON public.sitesnap_audit_logs(entity_type, entity_id);

ALTER TABLE public.sitesnap_audit_logs ENABLE ROW LEVEL SECURITY;

CREATE POLICY sitesnap_audit_logs_select ON public.sitesnap_audit_logs
  FOR SELECT USING (
    public.sitesnap_has_permission(company_id, 'sitesnap.export_reports')
    OR company_id IN (
      SELECT ce.company_id FROM public.core_employees ce
      WHERE ce.auth_user_id = auth.uid() AND ce.role IN ('owner', 'admin', 'administrator')
    )
...
CREATE POLICY sitesnap_audit_logs_insert ON public.sitesnap_audit_logs
...
-- ----------------------------------------------------------------------------
REVOKE ALL ON FUNCTION public.sitesnap_default_permissions(text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.sitesnap_current_employee(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.sitesnap_current_employee_id(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.sitesnap_has_permission(uuid, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.sitesnap_module_enabled(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.sitesnap_my_permissions(uuid) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.sitesnap_default_permissions(text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.sitesnap_current_employee(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.sitesnap_current_employee_id(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.sitesnap_has_permission(uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.sitesnap_module_enabled(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.sitesnap_my_permissions(uuid) TO authenticated;
