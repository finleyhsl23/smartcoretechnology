...
export const tasks = {
  async listForProject(projectId) {
    const { data, error } = await sb().from("sitesnap_tasks")
      .select("*, core_employees!sitesnap_tasks_assignee_employee_id_fkey(full_name)").eq("project_id", projectId).order("created_at", { ascending: false });
    throwIfError(error);
    return data || [];
  },
  async listForCompany(companyId, { assigneeId = null, status = null } = {}) {
    let q = sb().from("sitesnap_tasks")
      .select("*, sitesnap_projects(name), core_employees!sitesnap_tasks_assignee_employee_id_fkey(full_name)")
