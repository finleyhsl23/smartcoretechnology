...
export async function requireSiteSnapAccess() {
  ...
  if (error || !mod?.enabled) {
    renderBlockScreen({
      ...
      message: "SiteSnap has not been enabled for your company.",
  ...
  if (!permissions.length) {
    renderBlockScreen({
      icon: "🚫",
      title: "No Access",
      message: "You don't have any SiteSnap permissions yet. Ask an owner or administrator to grant you access.",
    });
...
export async function getMyPermissions(companyId) {
  ...
  const { data, error } = await sb().rpc("sitesnap_my_permissions", { p_company_id: companyId });
