 ...
 *  3. Company entitlement -> company_modules.enabled for 'convoy'
 *  4. Caller holds at least one Convoy permission
 */
export async function requireConvoyAccess() {
  let profile;
  try {
  ...
  if (error || !mod?.enabled) {
    renderBlockScreen({
      ...
      message: "Convoy has not been enabled for your company.",
  ...
  if (!permissions.length) {
    renderBlockScreen({
      icon: "🚫",
      title: "No Access",
      message: "You don't have any Convoy permissions yet. Ask an owner or administrator to grant you access.",
    });
    throw new Error("No permissions");
  }

  return { profile, permissions, admin: isAdmin(profile) };
}

export async function getMyPermissions(companyId) {
  if (_permissions) return _permissions;
  const { data, error } = await sb().rpc("convoy_my_permissions", { p_company_id: companyId });
